    ...
    document.addEventListener('mousemove', event => {
        if(mouseDownSwipable){
            requestAnimationFrame(() => {
                ...
                item.style.transform = `scale(110%) translate(-50%, ${-45 + (Math.abs(startX - event.clientX)/25)}%) rotate(${Math.min(mouseSpeed, 100) * (event.clientX - lastMouseX < 0 ? -1 : 1) + (startX - event.clientX)/-75}deg) rotateY(${(startX - event.clientX) / 10}deg) skew(${(startX - event.clientX)/-75}deg, ${(startX - event.clientX)/-75}deg)`;
